...
const AddUserRecipe: React.FC = () => {
  ...
  useEffect(() => {
    if (recipe) {
      setFormValues({
        ...
        approved: recipe.approved || false,
      });
    }
  }, [recipe]);
  ...
  const validateForm = () => {
    ...
    if (!formValues.recipe_steps_eng && activeTab == 'eng') {
      errors.recipe_steps_eng = 'Recipe steps in English';
    }
    if (!formValues.recipe_steps_hindi && activeTab == 'hindi') {
      errors.recipe_steps_hindi = 'Recipe steps in Hindi';
    }
    if (!formValues.recipe_steps_guj && activeTab == 'guj') {
      errors.recipe_steps_guj = 'Recipe steps in Gujarati';
    }
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      notify(
        id
          ? 'Recipe updated successfully wait untill admin approve your recipe'
          : 'Recipe added successfully wait untill admin approve your recipe',
        {
          type: 'success',
        },
      );
  ...
  return (
    <div className="bg-white">
      ...
      <form className="space-y-4 lg:ps-20 lg:pe-20" onSubmit={handleSubmit}>
          ...
          {activeTab === 'eng' && (
            <div className="  ">
              <div>
                ...
                />
                {formErrors.recipe_name_eng && (
                  <p className="text-red-500">{formErrors.recipe_name_eng}</p>
                )}
              </div>
              <div className="mt-2">
                <label className="block font-bold ">
                ...
                />
                {formErrors.ingredients_eng && (
                  <p className="text-red-500">{formErrors.ingredients_eng}</p>
                )}
              </div>
              <div className="mt-2">
                <label className="block font-bold">
                  Recipe Steps<span className="text-red-500">*</span>
                </label>
                <ReactQuill
                  ...
                  className="rounded data"
                />
                {formErrors.recipe_steps_eng && (
                  <p className="text-red-500">{formErrors.recipe_steps_eng}</p>
                )}
              </div>
            </div>
          )}
          {activeTab === 'hindi' && (
            <div className="">
              <div>
                ...
                />
                {formErrors.recipe_name_hindi && (
                  <p className="text-red-500">{formErrors.recipe_name_hindi}</p>
                )}
              </div>
              <div className="mt-2">
                ...
                <ReactQuill
                  ...
                  className="rounded data"
                />
                {formErrors.ingredients_hindi && (
                  <p className="text-red-500">{formErrors.ingredients_hindi}</p>
                )}
              </div>
              <div className="mt-2">
          ...
          {activeTab === 'guj' && (
            <div className="">
              <div>
                ...
                />
                {formErrors.recipe_name_guj && (
                  <p className="text-red-500">{formErrors.recipe_name_guj}</p>
                )}
              </div>
              <div className="mt-2">
                <label className="block  font-bold">Ingredients<span className="text-red-500">*</span></label>
                <ReactQuill
                  ...
                  className="rounded data"
                />
                {formErrors.ingredients_guj && (
                  <p className="text-red-500">{formErrors.ingredients_guj}</p>
                )}
              </div>
              <div className="mt-2">
                <label className="block  font-bold">Recipe Steps<span className="text-red-500">*</span></label>
                <ReactQuill
                  ...
                  className="rounded data"
                />
                {formErrors.recipe_steps_guj && (
                  <p className="text-red-500">{formErrors.recipe_steps_guj}</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              type="file"
              accept=".jpg,.jpeg,.png"
              className="w-full border border-[#ccc] rounded p-2 cursor-pointer"
              onChange={(e) => {
          ...
          <div>
            ...
            <input
              ...
              onChange={handleInputVIDEOChange as any}
        ...
        {preview || formValues.images ? (
          <div>
            <img
              className="w-60 h-auto mb-6 object-cover"
              src={
                preview || `http://localhost:3000/${formValues.images || ''}`
              }
              alt="recipe-image"
            />
          </div>
